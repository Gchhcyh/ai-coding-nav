        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-6 sm:p-8">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`px-2.5 py-1 rounded text-xs font-medium border ${
                    categoryColors[tool.category] || "bg-gray-700/50 text-gray-400 border-gray-600"
                  }`}
                >
              ...
              {tool.featured && (
                <span className="px-2 py-1 rounded text-xs font-bold bg-primary-600 text-white shrink-0">
                  精选
                </span>
              )}
            </div>
            ...
            {tool.stars > 0 && (
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-sm text-gray-300">
                  {tool.stars.toLocaleString()} GitHub Stars
                </span>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-500 text-white font-medium transition-colors text-sm"
              >
                访问官网
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gray-700/50 hover:bg-gray-600/50 text-gray-300 font-medium transition-colors text-sm border border-gray-600"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="mb-6">
              <ToolDetailList
                best_for={(tool as any).best_for}
                not_for={(tool as any).not_for}
              />
            </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tool.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 rounded text-xs bg-gray-700/50 text-gray-400 border border-gray-600"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {sameCategoryTools.slice(0, 6).map((t: any) => (
                  <Link
                    key={t.slug}
                    href={`/tools/${t.slug}`}
                    className="block p-4 rounded-xl bg-gray-800/30 border border-gray-700/50 hover:border-gray-600 transition-colors"
                  >
          ...
          <div className="mt-8">
            <Link
              href="/recommend"
              className="block p-5 rounded-xl bg-gradient-to-r from-purple-900/20 to-primary-900/20 border border-purple-800/30 hover:border-purple-700/50 transition-all group text-center"
            >
